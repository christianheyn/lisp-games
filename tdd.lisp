...
(defvar *tdd-error-length* 0)

(defun inc-error-length () (setf *tdd-error-length* (+ *tdd-error-length* 1)))
  ...
  (if skip-check
    ...
    (if true-check
      (let* ((format-fn (if is-true #'format-green #'format-red)))
            ...
            (when (not is-true) (inc-error-length))
            (terpri t))
      (let* ((result (equal actual expected))
            ...
            (format t (funcall format-fn test))
            (terpri t)
            (when (not result) (inc-error-length))
            (when (not result)
                  (progn
